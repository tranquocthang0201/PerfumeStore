...
async function loadInitialProducts() {
    try {
        ...
        console.log("Dữ liệu lấy từ backend:", { products, brands, categories });
    } catch (error) {
        console.error("Lỗi khi lấy dữ liệu từ backend:", error);
        products = products.length ? products : DEFAULT_PRODUCTS_DATA;
        brands = brands.length ? brands : [];
        categories = categories.length ? categories : [
            { name: "Nước hoa Nam", slug: "nam" },
            { name: "Nước hoa Nữ", slug: "nu" },
            { name: "Nước hoa Unisex", slug: "unisex" }
        ];
    }
...
async function refreshCatalogFromBackend() {
    if (catalogRefreshPromise || Date.now() - lastCatalogRefreshAt < 3000) return catalogRefreshPromise;

    catalogRefreshPromise = Promise.all([
        apiGet("/products"),
        apiGet("/brands"),
        apiGet("/categories")
    ]).then(([latestProducts, latestBrands, latestCategories]) => {
        ...
        console.error("Không thể tự làm mới dữ liệu trang chủ:", error);
    }).finally(() => {
...
window.addEventListener("focus", refreshCatalogFromBackend);
document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "visible") refreshCatalogFromBackend();
});
  ...
  function brandName(slug) {
      return brands.find(brand => brand.slug === slug)?.name || String(slug || '').toUpperCase();
  }

  function categoryName(slug) {
      return categories.find(category => category.slug === slug)?.name || String(slug || 'Không rõ');
  }

  function brandVisual(brand, imageClass = '') {
      const logo = String(brand.logo || '').trim();
      if (logo) {
          return `<img src="${escapeHtml(logo)}" alt="${escapeHtml(brand.name)}" class="${imageClass}" onerror="this.style.display='none';this.nextElementSibling.style.display='inline-flex'"><span class="brand-text-logo" style="display:none">${escapeHtml(brand.name)}</span>`;
      }
      return `<span class="brand-text-logo">${escapeHtml(brand.name)}</span>`;
  }

  function bindFilterEvents() {
      document.querySelectorAll('aside input').forEach(input => {
          input.onchange = () => { currentPage = 1; renderList(); };
      });
  }

  function renderCatalogMetadata() {
      const selectedTypes = new Set(Array.from(document.querySelectorAll('input[name="loai"]:checked')).map(input => input.value));
      const selectedBrands = new Set(Array.from(document.querySelectorAll('input[name="thuonghieu"]:checked')).map(input => input.value));

      const categoryFilter = document.getElementById('category-filter-list');
      if (categoryFilter) {
          categoryFilter.innerHTML = categories.length
              ? categories.map(category => `
                  <div class="form-check">
                      <input class="form-check-input" type="checkbox" value="${escapeHtml(category.slug)}" name="loai" id="category-${escapeHtml(category.slug)}" ${selectedTypes.has(category.slug) ? 'checked' : ''}>
                      <label class="form-check-label" for="category-${escapeHtml(category.slug)}">${escapeHtml(category.name)}</label>
                  </div>`).join('')
              : '<div class="text-muted small">Chưa có danh mục</div>';
      }

      const brandFilter = document.getElementById('brand-filter-list');
      if (brandFilter) {
          brandFilter.innerHTML = brands.length
              ? brands.map(brand => `
                  <div class="form-check">
                      <input class="form-check-input" type="checkbox" value="${escapeHtml(brand.slug)}" name="thuonghieu" id="brand-${escapeHtml(brand.slug)}" ${selectedBrands.has(brand.slug) ? 'checked' : ''}>
                      <label class="form-check-label" for="brand-${escapeHtml(brand.slug)}">${escapeHtml(brand.name)}</label>
                  </div>`).join('')
              : '<div class="text-muted small">Chưa có thương hiệu</div>';
      }

      const track = document.getElementById('brand-track');
      if (track) {
          if (!brands.length) {
              track.innerHTML = '<div class="brand-slide text-muted">Chưa có thương hiệu</div>';
              track.style.setProperty('--brand-count', 1);
          } else {
              const repeated = [...brands, ...brands];
              track.style.setProperty('--brand-count', brands.length);
              track.innerHTML = repeated.map(brand => `
                  <div class="brand-slide" data-brand="${escapeHtml(brand.slug)}" title="${escapeHtml(brand.name)}">
                      ${brandVisual(brand)}
                  </div>`).join('');
          }
      }
      ...
      if (grid) {
          grid.innerHTML = brands.length
              ? brands.map(brand => `
                  <div class="col">
                      <div class="brand-card shadow-sm p-4 text-center rounded-3 bg-white border h-100" data-brand="${escapeHtml(brand.slug)}" style="cursor:pointer">
                          <div class="brand-grid-logo mb-3">${brandVisual(brand, 'img-fluid')}</div>
                          <h5 class="fw-bold text-uppercase mb-1">${escapeHtml(brand.name)}</h5>
                          <small class="text-muted">${Number(brand.productCount || 0)} sản phẩm</small>
                      </div>
                  </div>`).join('')
              : '<div class="col-12 text-center text-muted">Chưa có thương hiệu</div>';
      }

      document.querySelectorAll('[data-brand]').forEach(element => {
          element.onclick = () => window.filterByBrand(element.dataset.brand);
      });
      bindFilterEvents();
  }
  ...
  window.showProductDetail = function(productId) {
    ...
    if(brandTop) brandTop.textContent = brandName(p.brand);
    ...
    if(specBrand) specBrand.textContent = brandName(p.brand);
    const specType = document.getElementById('spec-type');
    if(specType) specType.textContent = categoryName(p.type);
  ...
  function createProductCard(p) {
      ...
      div.innerHTML = `
        <div class="product-card-minimal position-relative h-100 pb-2 shadow-sm bg-white rounded" onclick="window.handleProductClick('${p.id}')" style="cursor: pointer;">
            ${Number(p.discount || 0) > 0 ? `<span class="product-promo-badge product-promo-badge--sale" aria-label="Giảm ${Number(p.discount)} phần trăm"><span>GIẢM ${Number(p.discount)}%</span></span>` : ''}
            ${p.isNew ? `<span class="product-promo-badge product-promo-badge--new" aria-label="Sản phẩm mới"><span>MỚI</span></span>` : ''}
            ...
            <div class="card-body p-2 text-center">
                <div class="text-muted small text-uppercase mb-1">${escapeHtml(brandName(p.brand))}</div>
                <div class="card-title fw-bold text-dark mb-1 text-truncate">${escapeHtml(p.name)}</div>
  ...
  function renderExtraSections() {
      ...
      const saleItems = products
          ...
          .sort((a, b) => Number(b.discount || 0) - Number(a.discount || 0) || Number(b.dbId || 0) - Number(a.dbId || 0))
          .slice(0, 4);
      if(saleContainer) {
      ...
      const newItems = products
          ...
          .sort((a, b) => Number(b.dbId || 0) - Number(a.dbId || 0))
          .slice(0, 4);
      if(newContainer) {
...
window.renderMyOrders = async function() {
    ...
    try {
        myOrders = await apiGet("/orders/my");
    } catch (error) {
...
if (formLogin) {
  ...
  // Hồ sơ được đọc và cập nhật qua API; localStorage chỉ lưu phiên đăng nhập.
  ...
  function storeCurrentUser(user) {
      ...
      localStorage.setItem('loggedInUser', user.fullName || user.email);
      localStorage.setItem('loggedInEmail', user.email);
      loggedInUser = user.fullName || user.email;
      updateLogin();
  }
  ...
  window.renderProfile = async function() {
      ...
      try {
          ...
          alert(error.message || 'Không tải được hồ sơ. Vui lòng đăng nhập lại.');
      }
  };
  ...
  if (formProfile) {
      formProfile.addEventListener('submit', async function(e) {
          ...
          try {
              const result = await apiPut('/users/me', {
 ...
 // Sửa lại showPage để load dữ liệu khi vào trang hồ sơ
  ...
  // Đồng bộ các thao tác đơn hàng với REST API thay vì localStorage.
  ...
  window.viewUserOrderDetail = async function(orderId) {
      try {
          const order = await apiGet(`/orders/${orderId}`);
          document.getElementById('u-od-id').textContent = `Chi tiết đơn hàng #${order.id}`;
          document.getElementById('u-od-name').textContent = order.customerName || '';
          document.getElementById('u-od-phone').textContent = order.phone || '';
          document.getElementById('u-od-addr').textContent = order.address || '';
          document.getElementById('u-od-date').textContent = order.date || '';
          document.getElementById('u-od-payment').textContent = order.paymentMethod || 'COD';
          ...
          listEl.innerHTML = (order.items || []).map(item => `
            <tr class="border-bottom">
              <td class="ps-3"><div class="fw-bold text-dark small">${item.productName}</div><small class="text-muted">${item.ml}ml</small></td>
              <td class="text-center">${formatVND(item.price)}</td>
              <td class="text-center">${item.quantity}</td>
              <td class="text-end pe-3 fw-bold">${formatVND(item.price * item.quantity)}</td>
            </tr>`).join('');

          bootstrap.Modal.getOrCreateInstance(document.getElementById('userOrderDetailModal')).show();
      } catch (error) {
